...
energyUsageData <- read.table("household_power_consumption.txt",skip = startPos,nrows = endPos-startPos,sep = ";",col.names = columnNames)

# Create the output image file and write the plot to that file
png(filename="plot1.png")
with(energyUsageData,hist(Global_active_power,col = "red",xlab = "Global Active Power (kilowatts)",main = "Global Active Power"))
dev.off()